...
import { loadSnapshotDataIntoContract } from "../actions/load-snapshot-data";
...
const main = async () => {
  ...
  if (!deployFilename || deployFilename.substring(0, 11) !== "deployment-") {
    console.log(`Usage : load-snapshot-data.ts <deploy_cache_filename>`);
    process.exit(1);
  }
  ...
  await loadSnapshotDataIntoContract({
    deployedContracts,
    config,
    env,
  });
