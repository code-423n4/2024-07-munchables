...
import { verifyStorageSlotDataFromContract } from "../actions/verify-snapshot-data";
...
const main = async () => {
  ...
  if (!deployFilename || deployFilename.substring(0, 11) !== "deployment-") {
    console.log(`Usage : verify-storage-slots.ts <deploy_cache_filename>`);
    process.exit(1);
  }
  ...
  await verifyStorageSlotDataFromContract({
    deployedContracts,
    config,
  });
};
