...
describe("ClaimManager: claimPoints", () => {
  let admin: `0x${string}`;
  let bob: `0x${string}`;
  ...
  before(async () => {
    ...
    [bob, alice, jirard] = testRoleAddresses.users;
  ...
  describe("all paths", () => {
    ...
    it("claim, successive calls, zero points", async () => {
      let txHash = await testContracts.claimManagerProxy.contract.write.claimPoints({
        account: alice,
      });
    ...
    it("revert claim outside of valid period", async () => {
      ...
      await assert.rejects(
        testContracts.claimManagerProxy.contract.simulate.claimPoints({
          account: alice,
        }),
        (err: Error) => assertContractFunctionRevertedError(err, "InvalidPeriodError")
      );
    });
  });
});
