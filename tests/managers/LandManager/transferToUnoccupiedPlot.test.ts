...
describe("LandManager: transferToUnoccupiedPlots", () => {
  let alice: `0x${string}`;
  let bob: `0x${string}`;
  ...
  describe("all paths", () => {
    ...
    it("not staked token id", async () => {
      await assert.rejects(
        testContracts.landManagerProxy.contract.simulate.transferToUnoccupiedPlot([2, 1], {
          account: bob,
        }),
    ...
    it("invalid owner", async () => {
      await assert.rejects(
        testContracts.landManagerProxy.contract.simulate.transferToUnoccupiedPlot([1, 3], {
          account: jirard,
        }),
        (err: Error) => assertContractFunctionRevertedError(err, "InvalidOwnerError")
      );
    });
    it("occupied plot", async () => {
      const txHashApprove = await testContracts.munchNFT.contract.write.approve(
      ...
      await assertTxSuccess({ txHash: stakeMunchableTxHash });
      await assert.rejects(
        testContracts.landManagerProxy.contract.simulate.transferToUnoccupiedPlot([1, 119], {
          account: bob,
        }),
    ...
    it("plot too high", async () => {
      await assert.rejects(
        testContracts.landManagerProxy.contract.simulate.transferToUnoccupiedPlot([1, 150], {
          account: bob,
        }),
